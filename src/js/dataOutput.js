...
// read local storage
for (let i = 0; i < localStorage.length; i++) {
  var key = localStorage.key(i); //for each recipe that we store a key is set with which we can recognize the recipe. The keys are listed in an array and they can be called by their index number.
